...
#NOTE1: 上面的模式效率更高.
#NOTE2: 这种模式会覆盖select_y_key，会覆盖其他模式。如果想要其不生效直接置空即可：select_y_key_multi_line=''
select_y_key_multi_line='entry.processCostTimeMax,entry.processCostTimeAvg entry.encoderCostTimeAvg,entry.encoderCostTimeMax'

#对于极其复杂的数据格式，可以用yaml的格式来配置
#比如文本为： cpu_info: 1/20/30,2/15/25 mem_info: 100_200_300
#cpu_info有两组数组数据用逗号分隔，每个数据用斜杠分隔，分别代表最小值/平均值/最大值
#mem_info有一组数据用下划线分隔
#对于这种情况可以用下面的yaml格式来配置
#cpu_info: 代表选择cpu_info为key的数据
#  split_reg: [',', '/']  代表先用逗号分隔成两组数据，然后每组数据用斜杠分隔
#  select: 代表选择哪些数据
#    - name: name1  代表第一组数据的名字
#      pos: [1, 1]  代表第一组数据的第一个值
#      group: group1  代表name1归为group1组
#    - name: name2  代表第一组数据的名字
#      pos: [2, 2]  代表第一组数据的第二个值
#      group: group2  代表name2归为group2组
#mem_info: 代表选择mem_info为key的数据
#  split_reg: ['_',]  代表用下划线分隔
#  select: 代表选择哪些数据
#    - name: name3  代表第一组数据的名字
#      pos: [1]  代表第一组数据的第一个值
#      group: group1  代表name3归为group1组
#group_order: ['group1', 'group2']  代表group1是第一张图，group2是第二张图
#NOTE: 这种模式会覆盖selete_y_raw, select_y_key_multi_line
select_y_with_yaml_format="""
keys:
  processorCpuStr:
    split_reg: [',', '/']
    select:
  ...
  AudioEncoderQueue:
    split_reg: [',', '/']
    select:
      - name: encoder_queue_cpu
        pos: [-1, 1]
        group: group2
  AudioRecordThread:
    split_reg: [',', '/']
    select:
      - name: audio_record_thread_cpu
        pos: [-1, 1]
        group: group3
group_order: ['group1', 'group3']
"""
...
#组合模式下的message是否是json格式
key_message_is_json=False

##当检测到file为CSV格式，遵循以下规则
#select_key_y_csv=['A,B', 'C']
#select_filter_key_csv='M,N:1'
# ['A,B', 'C'] 表示选择header分别为A, B, C的三列, 其中AB在一张图上，C在另一张图上
# M,N:1 表示将C,D两列的值联合起来做为KEY，来将整张表分类,
###### 后面的1 表示将分类后的表画在同一张图上，0表示不要画在同一个图上
#所以上面的意思是，将整张表按照header为M,N的列分类，并将分类后的表里面取出A,B,C列数据，
#然后分别将分类后的数据画在不同的图上，并且每个分类后的数据，又按照A,B一张图，C另一张图来分开画
select_key_y_csv=['PSNR Mean', 'PSNR Std', 'VMAF Mean']
select_filter_key_csv='Width:1'
#强制按照txt文本来解析
force_read_as_txt=False

#是否使用组合模式
#使用组合模式后,跟文本模式唯一的区别就是先选取csv中的某一列作为文本，然后将这个文本作为
#按照txt文本来解析
enable_combination_mode=True
#组合模式下选择的文本message,这个关键字只在组合模式下生效
combination_message='attributes'


#select_key_x_csv长度只能为0,1或者跟select_key_y_csv一样
##### 0 代表使用默认X轴
##### 1 代表select_key_y_csv都是使用同一个X列
##### 同长度 代表select_key_y_csv的每一个key都有对应相同index下选择的X
#可以用null填充,null表示使用默认X轴
#比如： select_key_x_csv = ['X1', 'null', 'X2']
select_key_x_csv=['Bitrate']
#==================================================选择X轴Y轴的数据====================================================
...
#==================================================数据处理设置=====================================================
#这个选项是用来设置数据的缩放比例比如data_scale_factor='0.01,0.1 0.01 0.01'
#代表第一列第一部分数据缩放0.01，第二部分数据缩放0.1，
#第二列数据缩放0.01，第三列数据缩放0.01
#格式与select_y_key格式一样
data_scale_factor=''
#==================================================数据处理设置=====================================================
...
plot_script_path = current_dir + "/plot"
...
param_dict= {
    ...
    "key_message_is_json":key_message_is_json,
    'select_key_y_csv':select_key_y_csv,
    'select_key_x_csv':select_key_x_csv,
    'select_filtFalseer_key_csv':select_filter_key_csv,
    "force_read_as_txt":force_read_as_txt,
    "enable_combination_mode":enable_combination_mode,
    "combination_message":combination_message,
...
parser.add_argument("-f", "--file", help="File to analyze optional")
args = parser.parse_args()
if args.file is not None:
    param_dict["file_path"] = args.file
else:
    param_dict["file_path"] = "./log/lark_original.csv"

plot_engine = run_plot.PlotEngine()
plot_engine.init(param_dict)
plot_engine.plot()
plot_engine.exit()
